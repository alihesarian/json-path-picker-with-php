  ...
  if (typeof json === 'string') {
    ...
    if (json.length > 0) {
      ...
      for (var i = 0; i < json.length; i += 1) {
        let uuid = uuidv4();
        html += "<li data-key-type=\"array\" data-uuid='"+uuid+"' data-key=\"".concat(i, "\">"); // Add toggle button if item is collapsable
...
function PickPathHandler(elm) {
  ...
  for (var i = 0; i < $parentsList.length; i += 1) {
    ...
    if(keyType == 'array' && $parentsList[i].dataset.uuid != undefined){
      for (let x = 0; x < $parentsList[i].querySelectorAll("li ul.json-dict li").length; x++) {
        let item = $parentsList[i].querySelectorAll("li ul.json-dict li")[x];
        let key = item.dataset.key;
        let html = '';
        if(item.querySelector('.json-string')){
          html = item.querySelector('.json-string').innerHTML;
        }else if(item.querySelector('.json-literal')){
          html = item.querySelector('.json-literal').innerHTML;
        }
        ElementsKey.push({
          key: key,
          html: html
  ...
  pathSegments = pathSegments.map(function (segment, idx) {
    ...
    if (segment.keyType === 'array' || segment.isKeyANumber) {
      let ConditionKey = false;
      let ConditionHtml = false;
      if(segment.condition){
        let PromptKey = prompt("Please enter the condition key", segment.condition[0].key);
        if (PromptKey != null || PromptKey != "") {
          ConditionKey = PromptKey;
        }else{
          ConditionKey = segment.condition[0].key;
        }

        let PromptHtml = prompt("Please enter the condition value", segment.condition[0].html);
        if (PromptHtml != null || PromptHtml != "") {
          ConditionHtml = PromptHtml;
        }else{
          ConditionHtml = segment.condition[0].html;
        }
      }
      if(ConditionKey != false && ConditionHtml != false){
        return `[?(@.${ConditionKey} == ${ConditionHtml})]`;
      }
